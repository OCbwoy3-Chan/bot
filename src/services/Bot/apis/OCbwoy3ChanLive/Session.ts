...
export class LiveSession {
	...
	public async beginSession(model: string = "gemini-2.0-flash-live-001") {
		...
		player.on(AudioPlayerStatus.Playing, () =>
			console.log("Live Session: Playing")
		);
		...
		this.connection.receiver.speaking.on("start", (userId: string) => {
				...
				try {
					...
					this.session!.sendRealtimeInput({
						media: {
							...
							mimeType:
								"audio/pcm; rate=48000; channels=2; bit-depth=16; endianness=little"
						}
					});
					...
					if (!this.session || this.plsDie) {
						console.warn(
							`Session closed while trying to send audio for user ${userId}`
						);
						...
						try {
							...
							console.warn(
								`Error destroying subscription for ${userId} during send:`,
								e
							);
						}
			...
			sub.on("error", (err) => {
				// Add error handler for the subscription itself
				console.error(
					`AudioReceiveStream error for user ${userId}:`,
					err
				);
				...
				try {
					...
					console.warn(
						`Error destroying subscription for ${userId} on error:`,
						e
					);
				}
				activeUsers.delete(userId);
			...
			const onEnd = (endedUserId: string) => {
				if (endedUserId === userId || this.plsDie === true) {
					...
					try {
						...
						console.warn(
							`Error destroying subscription for ${userId} on end:`,
							e
						);
	...
	public async endSession() {
		assert(
			!!this.session || this.plsDie,
			"Session already ended or ending"
		); // Allow if plsDie is true
